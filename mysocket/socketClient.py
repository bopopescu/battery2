...
class myConfig(object):
    def setConfig(self, type=None, cellid=None, ip=None, port=None, boxid=None, chnnum=None, cmd=None, plan=None,
                  timeout=1, waittime=0.2, length=100, senddata=None, recvdata=None, testid=None, planid=None):
        self.type = type
        self.cellid = cellid
        self.ip = ip
        self.port = port
...
class socketConnect(object):
    ...
    def buildCmdMessage(self, config):
        ...
        if (config.type == 'box'):
            if (config.cmd == 'start'):
                ...
                # 构造命令
                readBoxDetailDataCMD = bytearray(
                    [0xAA, 0x55, 0x0C, 0x00, 0x00, 0x00, 0x06, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0F, 0x00,
                     0x55, 0xAA])
                pass
            elif (config.cmd == 'readResultData'):
                # 构造命令
                readBoxResultDataCMD = bytearray(
                    [0xAA, 0x55, 0x0C, 0x00, 0x00, 0x00, 0x06, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0F, 0x00,
                     0x55, 0xAA])
                pass
            else:
    ...
    def mainProcess(self):
        ...
        while True:
            ...
            #1.查询数据库，提取出需要进行操作的电炉子，即现有状态不等于下一个状态
            #2.查询数据库，提取出需要进行操作的电池，即现有状态不等于下一个状态
            #3.查询数据库，提取需要进行操作的流量计，即现有状态不等于下一个状态
            ...
            # 炉子控制的主逻辑
            for i in range(len(ovenUnderHandle)):
                # 温控器
                if (ovenUnderHandle[i]['currOvenState'] == 'stop') and (ovenUnderHandle[i]['nextOvenState'] == 'start'):
                    ...
                    pass
                elif (ovenUnderHandle[i]['currOvenState'] == 'start') and (
                        ovenUnderHandle[i]['nextOvenState'] == 'pause'):
                    # 构建炉子的报文
                    # 发送炉子报文
                    # 更新实时状态表ovendevicetable
                    pass
                elif (ovenUnderHandle[i]['currOvenState'] == 'start') and (
                        ovenUnderHandle[i]['nextOvenState'] == 'start'):
                    # 读炉子的温度
                    # 存数据commonhistory
                    # 读流量计数据
                    # 存数据commonhistory
            ...
            #流量计控制的主逻辑
            for iGas in range(len(lljUnderHandle)):
            ...
            for i in range(len(cellsUnderHandle)):
                for j in range(len(cellsComponetCOM)):
                    if cellsUnderHandle[i]['cellID_id'] == cellsComponetCOM[j]['cellID']:
                        ...
                        # 流量计和温度计读数
                        boxState = (cellsUnderHandle[i]['currState'] == "start") and (
                                    cellsUnderHandle[i]['nextState'] == "start")
                        ovenState = (cellsUnderHandle[i]['currOvenState'] == "start") and (
                                    cellsUnderHandle[i]['nextOvenState'] == "start")
                        ...
                        # 电子负载
                        if (cellsUnderHandle[i]['currState'] == "start") and (
                                cellsUnderHandle[i]['nextState'] == "start"):
                            # 电子负载
                        ...
                        elif (cellsUnderHandle[i]['currState'] == "start") and (
                                cellsUnderHandle[i]['nextState'] == "pause"):
                            # 电子负载
                            print('box:pause')
                            config.setConfig(type='box',
                                             cellid=cellsUnderHandle[i]['cellID_id'],
                            ...
                            self.updateCellBoxState(config)
                                ...
                                cellsUnderHandle[i]['nextState'] == "stop"):
                            # 电子负载
                            print('box:stop')
                            config.setConfig(type='box',
                                             cellid=cellsUnderHandle[i]['cellID_id'],
                        ...
                        elif (cellsUnderHandle[i]['currState'] == "stop") and (
                                cellsUnderHandle[i]['nextState'] == "start"):
                            # 查找测试方案
                        ...
                        elif (cellsUnderHandle[i]['currState'] == "pause") and (
                                cellsUnderHandle[i]['nextState'] == "resume"):
                            # 电子负载
                            print('box:resume')
                            config.setConfig(type='box',
                                             cellid=cellsUnderHandle[i]['cellID_id'],
                        ...
                        elif (cellsUnderHandle[i]['currState'] == "resume") and (
                                cellsUnderHandle[i]['nextState'] == "stop"):
                            # 电子负载
                            print('box:stop')
                            config.setConfig(type='box',
