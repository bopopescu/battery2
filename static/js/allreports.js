...
function get_initialize() {
    ...
    $('#customer_tab').prop("checked",true);
...
function get_table() {
    ...
    optObj.date =$('[name=date]').val();
    var urlopt = $.param(optObj);
    var reportdata = get_report_data();
        $('#table').bootstrapTable({
            // url: '/getallreport/?' + urlopt,                           //请求后台的URL（*）
            // method: 'get',                     //请求方式（*）
            data:getrowdata(reportdata),
...
function getcolumns(reportdata) {
    var columns = [
                {
                    field: 'time',
                    title: '时间',
    ...
    for(i=0;i<reportdata.length;i++){
        if($.inArray(reportdata[i].sid, sidlist) === -1){
            var columone = {};
            columone['field'] = reportdata[i].sid;
            columone['title'] = reportdata[i].name;
            columone['align'] = 'center';
            columone['valign'] = 'middle';
            columone['sortable'] = true;
            sidlist.push(reportdata[i].sid);
            columns.push(columone);
        }
...
function  getrowdata(reportdata) {
    var alltime = [];
    for(i=0;i<reportdata.length;i++){
        if($.inArray(reportdata[i].time,alltime) === -1){
            alltime.push(reportdata[i].time);
        }
    }
    var rowdata = [];
    ...
    for(i=0;i<alltime.length;i++){
        var rowone = {};
        rowone['time'] = alltime[i];
        for(j=0;j<reportdata.length;j++){
            if(reportdata[j].time === rowone['time']){
                rowone[reportdata[j].sid] = reportdata[j].adde;
            }
        }
        var aa =rowone;
        rowdata.push(rowone);
    ...
    // for(i=0;i<reportdata.length;i++){
    //
    //     rowone['time'] = reportdata[i].time;
    //     rowone['sid'] = reportdata[i].adde;
    // }
